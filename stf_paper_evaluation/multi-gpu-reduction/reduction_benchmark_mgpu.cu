...
#include "mgpu-kernels/stf_pfor.cuh"
...
// Multi-GPU Benchmark function
void benchmark_reduction_mgpu(unsigned long long N, std::ofstream& csv_file, const std::string& model, bool verify_with_cpu = false, int num_iterations = 10, int num_gpus = 1) {
    ...
    std::cout << "Testing with " << num_gpus << " GPU(s):" << std::endl;
    ...
    if (model == "all" || model == "stf_pfor") {
        benchmark_stf_pfor_mgpu(N, d_X, ref_sum, verify_with_cpu, num_iterations, num_gpus, csv_file);
    }
...
int main(int argc, char** argv) {
    ...
    int num_gpus = 0; // 0 means test both 1 and 2 GPUs
    
    // Parse command line arguments
    while ((opt = getopt(argc, argv, "cm:n:i:g:")) != -1) {
        switch (opt) {
            case 'c':
            ...
            case 'g':
                ...
                if (num_gpus < 1 || num_gpus > 2) {
                    std::cerr << "Error: GPU count must be 1 or 2" << std::endl;
                    return 1;
                }
                break;
            default:
                std::cerr << "Usage: " << argv[0] << " [-c] [-m <model>] [-n <size>] [-i <iterations>] [-g <gpus>]" << std::endl;
                std::cerr << "  -c: Enable CPU reduction verification" << std::endl;
                std::cerr << "  -m: Benchmark specific model(s). Can be used multiple times. (stf_pfor, all). Default: all" << std::endl;
                ...
                std::cerr << "  -g: Number of GPUs (1 or 2). Default: test both 1 and 2 GPUs" << std::endl;
                return 1;
        }
    }
    ...
    // Determine GPU configurations to test
    std::vector<int> gpu_configs;
    if (num_gpus > 0) {
        gpu_configs.push_back(num_gpus);
    } else {
        // Test both 1 and 2 GPUs by default
        gpu_configs.push_back(1);
        if (device_count >= 2) {
            gpu_configs.push_back(2);
        }
    }
    ...
    for (unsigned long long N : problem_sizes) {
        for (int gpus : gpu_configs) {
            for (const std::string& model : models_to_run) {
                benchmark_reduction_mgpu(N, csv_file, model, verify_with_cpu, num_iterations_arg, gpus);
            }
            std::cout << std::endl;
        }
    }
