...
int main(int argc, char** argv)
{
  ...
  printf("Number of real devices: %d\n", real_devs);
  ...
  for (size_t b = 0; b < num_batches; b++)
  {
    ...
    // assemble the task symbol "Init_b"
    ctx.task(exec_place::device(0), data[b].write()).set_symbol("Init" + std::to_string(b))->*[](cudaStream_t, auto) {
      // Init ...
    };
  }
  ...
  for (size_t iter = 0; iter < niter; iter++)
  {
    for (size_t b = 0; b < num_batches; b++)
    {
      ...
      {
        ctx.task(exec_place::device(d % real_devs), data[b].rw()).set_symbol("Forward_" + std::to_string(b) + "_" + std::to_string(d%real_devs))->*[=](cudaStream_t s, auto bd) {
          int ms                  = 10;
      ...
      {
        ctx.task(exec_place::device(d % real_devs), data[b].rw()).set_symbol("Backward_" + std::to_string(b) + "_" + std::to_string(d%real_devs))->*[=](cudaStream_t s, auto bd) {
          int ms                  = 20;
          long long int clock_cnt = (long long int) (ms * clock_rate / factor);
