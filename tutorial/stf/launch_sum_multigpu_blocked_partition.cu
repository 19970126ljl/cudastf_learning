...
int main(int argc, char* argv[])
{
  ...
  } else if (number_devices == 1) {
    // Use single device execution
    where = exec_place::current_device();
    std::cout << "Using current device execution place" << std::endl;
  } else {
  ...
  auto perform_reduction = [&]() -> double {
    ...
    auto lsum = ctx.logical_data(make_slice(&result_sum, 1));
    auto host_result = ctx.logical_data(shape_of<scalar_view<double>>());
    lsum.set_symbol("lsum");

    auto spec = par<16>(con<32>());

    ctx.launch(spec, where, lX.read(), lsum.rw()).set_symbol("launch_sum")->*[] _CCCL_DEVICE(auto th, auto x, auto sum) {
      // Each thread computes the sum of elements assigned to it
      double local_sum = 0.0;

      // Use apply_partition to get the data partition assigned to this execution unit
    ...
    ctx.host_launch(lsum.rw(), host_result.write()).set_symbol("host_launch")->*[](auto sum, auto result) {
      *result = sum(0);
      printf("sum = %lf\n", sum(0));
      sum(0) = 0.0;
    };
    // double result = ctx.wait(host_result);
    ctx.finalize();
    return result;
  };
