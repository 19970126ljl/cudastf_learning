...
int main()
{
  ...
  auto lZ = ctx.logical_data(shape_of<slice<double>>(N));

  ctx.parallel_for(lZ.shape(), lZ.write())->*[alpha, beta, gamma]__device__(size_t i, auto dZ) {
    dZ(i) = alpha+beta+gamma + i;
  };
